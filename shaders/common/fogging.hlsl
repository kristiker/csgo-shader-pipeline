...
void ApplyFog(inout float3 pixelColor, float3 vPositionWs)
{
    ...
    // ApplyGradientFog
    {
        ...
        {
            float2 _21390 = clamp(mad(g_vGradientFogBiasAndScale.zw, float2(length(cameraCenteredPixelPos.xy), vPositionWs.z), g_vGradientFogBiasAndScale.xy), 0.0f.xx, 1.0f.xx);
    ...
    // ApplyCubemapFog
    {
        ...
        bool bPixelHasCubemapFog = (distanceQuadratic > g_vCubeFogCullingParams_MaxOpacity.x) && ((g_vCubeFogCullingParams_MaxOpacity.z * vPositionWs.z) < g_vCubeFogCullingParams_MaxOpacity.y);
        
        if (bPixelHasCubemapFog)
        {
            float _16990 = clamp(pow(max(0.0f, mad(length(cameraCenteredPixelPos.xy), g_vCubeFog_Offset_Scale_Bias_Exponent.y, g_vCubeFog_Offset_Scale_Bias_Exponent.x)), g_vCubeFog_Offset_Scale_Bias_Exponent.w), 0.0f, 1.0f) * clamp(pow(max(0.0f, mad(vPositionWs.z, g_vCubeFog_Height_Offset_Scale_Exponent_Log2Mip.y, g_vCubeFog_Height_Offset_Scale_Exponent_Log2Mip.x)), g_vCubeFog_Height_Offset_Scale_Exponent_Log2Mip.z), 0.0f, 1.0f);
